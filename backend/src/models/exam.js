...
module.exports.createExam = exam =>
  createQuestions(
    exam.questions.map(R.assoc("creatorId", exam.creatorId))
  ).then(questions => Exam.create({ ...exam, questions }).then(doc => doc._id));
  ...
  Exam.findOneAndUpdate(
    {
      _id: examId,
      $or: [
        { analyzing: { $exists: false } },
        { analyzing: { $lt: new Date(Date.now() - MAX_ANALYSIS_TIME) } }
      ],
      result: { $exists: false },
      endTime: { $lt: new Date() }
    },
